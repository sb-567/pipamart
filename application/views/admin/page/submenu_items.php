        ...
        <div class="col-md-6 col-md-offset-1 col-xs-12">
          <div class="col-sm-12">
            <div class="search_list">
              ...
              <div class="add_btn_primary"> <a href="<?php echo site_url("admin/submenu-item/add");?>?redirect=<?=$redirectUrl?>"><?=$this->lang->line('add_new_lbl')?></a> </div>
            </div>
          </div>
        </div>
      </div>
          ...
          <div class="col-lg-4 col-sm-6 col-xs-12 item_holder">
            <div class="block_wallpaper add_wall_category" style="box-shadow:0px 3px 8px rgba(0, 0, 0, 0.3)">  <div class="wall_category_block" style="z-index: 1">
                <h2 style="font-size: 16px">
                  ...
                  ?> - 
                  <?php 
                    if(strlen($row->sub_category_name) > 23){
                      echo substr(stripslashes($row->sub_category_name), 0, 23).'...';  
                    }else{
                      echo $row->sub_category_name;
                    }
                  ?>--
                  <?php 
                    if(strlen($row->submenu_header) > 23){
                      echo substr(stripslashes($row->submenu_header), 0, 23).'...';  
                    }else{
                      echo $row->submenu_header;
                    }
                  ?>
                </h2>
              ...
              <div class="wall_image_title">
                <h2><a href="<?php echo site_url("admin/sub-category/edit/".$row->id);?>?redirect=<?=$redirectUrl?>" title="<?=$row->submenu_header?>" style="text-shadow: 1px 1px 1px #000;font-size: 16px">
                  <?php 
                    if(strlen($row->submenu_item_name) > 20){
                      echo substr(stripslashes($row->submenu_item_name), 0, 20).'...';  
                    }else{
                      echo $row->submenu_item_name;
                    }
                  ?>
                </a></h2>
                <ul>                
                  <li><a href="<?php echo site_url("admin/submenu-item/edit/".$row->id);?>?redirect=<?=$redirectUrl?>" data-toggle="tooltip" data-tooltip="<?=$this->lang->line('edit_lbl')?>"><i class="fa fa-edit"></i></a></li>               
...
<script type="text/javascript">
  $(".btn_delete_a").click(function(e){
      ...
      e.preventDefault(); 
      var href = '<?=base_url()?>admin/SubMenu/delete_item/'+_id;
      var btn = this;
